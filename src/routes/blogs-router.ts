...
import { blogDescValidation, blogIdValidation, blogNameValidation,
        blogWebUrlValidation, blogWebUrlValidation2, inputValidationMiddleware} from "../validation/blogs-validation";
...
blogsRouter.get('/:id',
    blogIdValidation,
    (req: Request, res: Response) => {
        let findedBlog = blogs.filter(b => b.id === req.params.id)[0]
        if (findedBlog) {
